  ...
  methods: {
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request2.delete('/comment/delete/' + id).then(res => {
    ...
    addReplay(item) {
      ...
      this.$request2.post('/comment/add', { pid: item.id, rootId: item.rootId, content: item.replyContent, fid: this.fid, }).then(res => {
        if (res.code === 200) {
          this.$message.success('操作成功')
          item.replyContent = ''
    ...
    loadComment() {
      this.$request2.get('/comment/selectForUser', {
        params: {  fid: this.fid, module: this.module }
      }).then(res => {
        console.log("res",res)
        this.commentList = res.data || []
      })
      this.$request2.get('/comment/selectCount', {
        params: { fid: this.fid }
      }).then(res => {
    ...
    //评论
    addComment() {
      this.$request2.post('/comment/add', { content: this.commentContent, fid: this.fid}).then(res => {
        if (res.code === 200) {
          this.$message.success('操作成功')
          this.commentContent = ''
          this.loadComment()  // 重新加载数据
