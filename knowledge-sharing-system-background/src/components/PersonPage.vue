...
export default {
  ...
  methods: {
    getUserInfo() {
      this.$request2.get("/user/getPersonInfo").then(res=>{
    ...
    update() {
      ...
      // 保存当前的用户信息到数据库
      this.$request2.put('/user/updatePersonInfo', this.user).then(res => {
        if (res.code === 200) {
          // 成功更新
    ...
    save() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request2.put('/user/updatePassword', this.user).then(res => {
            if (res.code === 200) {
              // 成功更新
              // 成功更新
              localStorage.removeItem('user')   // 清除缓存的用户信息
              localStorage.removeItem('token')   // 清除缓存的用户信息
              localStorage.removeItem('role')   // 清除缓存的用户信息
